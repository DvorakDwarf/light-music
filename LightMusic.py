...
SSH(X)
Detect end(X)
Maybe change factor if first color is 255 for too long (X)
Night mode (Higher interval) (X)
Post on Github
Comments/Organize into function or files (kind of)
Overhaul radicalizer based on time and not interval(X)
...
#Constants
CHUNK = 256
INTERVAL = 0.02 #How long it waist between changing the lights
...
while loopActive:
    ...
    while True:
        ...
        if time.time() - intervalTimer >= INTERVAL:
            ...
            pi.set_PWM_dutycycle(secondPin, secondLight)
            pi.set_PWM_dutycycle(firstPin, firstLight)
